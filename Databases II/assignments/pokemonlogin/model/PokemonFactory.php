...
class PokemonFactory extends AbstractFactory {
    ...
    public function buscar($param) {
        ...
        {
            $busca = 'Ãgua';
        }else if($busca == 'Elétrico')
        {
            $busca = ElÃ©trico;
        }else if($busca == 'Dragão')
        {
            $busca = 'DragÃ£o';
        }
        
        try {
            $sth = $this->database->prepare("SELECT * FROM Pokemon WHERE LOWER(nome_pokemon) LIKE LOWER(:busca) OR LOWER(tipo) LIKE LOWER(:busca2)");
            
           // var_dump($busca);
            
            $sth->bindParam(":busca2", $busca);
