...
void divide_numgrandes(numerogrande *a, numerogrande *b, numerogrande *c)
{
	...
	if(compara_numgrandes(b, &zero) == 0)
	{
...
void resto_numgrandes(numerogrande *n1, numerogrande *n2, numerogrande *n3)
{
	numerogrande resto, quociente, q_d;
	inicializa_numgrande(&resto);
	inicializa_numgrande(&quociente);
	inicializa_numgrande(&q_d);
	divide_numgrandes(n1, n2, &quociente);
	multiplica_numgrandes(n2, &quociente, &q_d);
	subtrai_numgrandes(n1, &q_d, &resto);
	recebe_numgrandes(n3, &resto);
}
...
void repeated(numerogrande *n3, int alphabet[25])
{
	numerogrande aux, aux2, aux3;
	int i;
	inicializa_numgrande(&aux);
	inicializa_numgrande(&aux2);
	inicializa_numgrande(&aux3);
	for(i = 0; i < 25; i++)
	{
		if(alphabet[i] > 0)
		{
			int_to_numgrandes(alphabet[i], &aux);
			big_fact(&aux, &aux2);
			recebe_numgrandes(&aux3, n3);
			multiplica_numgrandes(&aux3, &aux2, n3);
		}
	}
}

void rep_permutation(int alphabet[25], int j, int max)
{
	numerogrande n1, n2, n3, result, remnder, final;
	...
	inicializa_numgrande(&final);
	remnder.digitos[0] = '7';
	remnder.digitos[9] = '1';
	remnder.ultimodigito = 9;
	n3.digitos[0] = '1';
	int_to_numgrandes(j, &n1);
	big_fact(&n1, &n2);
	repeated(&n3, alphabet);
	divide_numgrandes(&n2, &n3, &result);
	/*puts("resultado");
	imprime_numgrande(&n3);
	puts("passou?");*/
	resto_numgrandes(&result, &remnder, &final);
	/*puts("aqui nao ne?");*/
	imprime_numgrande(&final);
}

int main(void)
{
	char c;
	int alphabet[25], i, j, max;
	c = getchar();
	while(c != EOF)
	{
		j = max = 0;
		...
		rep_permutation(alphabet, j, max);
		c = getchar();
	}
	return 0;
}