...
void big_exp(numerogrande *n, int num, numerogrande *n3)
{
	int i;
	...
	recebe_numgrandes(&aux, n);
	for(i = 0; i < num-1; i++)
	{
		multiplica_numgrandes(n, &aux, &aux3);
		recebe_numgrandes(&aux, &aux3);
		recebe_numgrandes(n3, &aux3);
	}
}

int main(void)
{
	int N, a, b, i, rounds[1000];
	numerogrande xp, fact, xp1, fact1, result;
	scanf("%d", &N);
	for(i = 0; i < N; i++)
	{
		inicializa_numgrande(&xp);
		inicializa_numgrande(&xp1);
		inicializa_numgrande(&fact);
		inicializa_numgrande(&fact1);
		inicializa_numgrande(&result);
		scanf("%d^%d", &a, &b);
		int_to_numgrandes(a, &xp);
		big_exp(&xp, b, &result);
		imprime_numgrande(&result);
		scanf("%d!", &a);
		int_to_numgrandes(a, &fact);
		big_fact(&fact, &result);
		imprime_numgrande(&result);
	}
	return 0;
}