...
Modelo de PLI para ISP:

max x1 + x2 + ... + xn
s.a.:
   xi + xj  <= 1, para todo (i,j) in E
...
void foo_bar(glp_tree *tree, void *info)
{
  ...
  switch (glp_ios_reason(tree))
    ...
    case GLP_IHEUR:
      ...
      if(custo > bestPrimalBound)
        bestPrimalBound = custo;
...
{     glp_prob *lp;
  double z, valor, z_lp;
	...
	valor=glp_mip_col_val(lp, i+1); // recupera o valor da variavel i+1
      ...
      glp_write_lp(lp, NULL,"is.lp");
      glp_print_mip(lp, "is.sol");
#endif
