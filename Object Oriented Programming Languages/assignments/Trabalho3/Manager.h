...
void Manager<T>::readone(unsigned int ram_address, int n_worker, int index)
{
	...
	for(int i = 0; i < j; i++)
	{
		...
		/* Aqui, estamos procurando essa cache na lista de caches */
		if(n_worker == element)
		{
	...
	s.incMisses();
	/* Nesse proximo metodo, iremos checar o dado da unica cache que contem ele, que esta na 
	posiçao 0 */
	this->readmissone(ram_address, n_worker, index);
}
...
void Manager<T>::readtwo(unsigned int ram_address, int n_worker, int index)
{
	...
	/* Se nao achou - READMISS */
	s.incMisses();
	/* Nesse proximo metodo, faremos outra busca na lista de caches, visando achar o dado em outra cache; 
	 se a variavel z estiver em 0, significa que a cache que requisitou a leitura nao esta 
	 na lista, entao iremos procurar outra cache que talvez possua esse dado */
	this->readmisstwo(ram_address, n_worker, index, z);
}
...
void Manager<T>::writetwo(unsigned int ram_address, int n_worker, T value, int index)
{
	...
	s.incMisses();
	/* Nesse proximo metodo, faremos outra busca na lista de caches, visando achar o dado em outra cache - 
	 se a variavel element estiver em 0, significa que a cache que requisitou a escrita nao esta 
	 na lista, entao iremos procurar outra cache que talvez possua esse dado */
	this->writemisstwo(ram_address, n_worker, value, index, element);
}
...
void Manager<T>::readmisstwo(unsigned int ram_address, int n_worker, int index, int z)
{
	...
	if(z != 0)
	{
		element = d->getCacheList(z, index);
	}
	else
	{
		...
		{
			element = d->getCacheList(i, index);
			if(c[element]->checkTag(index, n))
			{
				i = d->returnNumberOfElements(index);
			}
		}
	}
	T mem;
